...
_risk_access = require_subscription(Features.ADVANCED_ANALYTICS)
...
async def get_risk_score(
    ...
    current_user: User = Depends(_risk_access),
):
    if portfolio_id:
        ...
        if not portfolios:
            return RiskScore(
                overall=5.0, diversification=5.0, concentration=5.0,
                position_size=5.0, volatility=5.0, correlation=5.0,
                recommendation="Create a portfolio to get personalized risk analysis.",
            )

        all_assets = []
        for p in portfolios:
            assets = await Asset.find(Asset.portfolio_id == p.id).to_list()
    ...
    scorer = RiskScorer()
    return scorer.calculate_portfolio_risk(portfolio)
...
async def run_stress_test(
    ...
    scenarios = {
        "recession": {"description": "Economic recession with 20-30% market decline", "impact_range": (-0.30, -0.20)},
        "market_crash": {"description": "Severe market crash like 2008 (40-50% decline)", "impact_range": (-0.50, -0.40)},
        "inflation": {"description": "High inflation environment (10-15% decline)", "impact_range": (-0.15, -0.10)},
        "interest_rate_hike": {"description": "Sharp interest rate increase (15-25% decline)", "impact_range": (-0.25, -0.15)},
    }
    ...
    for asset in portfolio.assets:
        asset_impact_multiplier = {"stock": 1.2, "etf": 1.0, "bond": 0.3, "crypto": 1.5, "reit": 1.1}.get(asset.asset_type.value, 1.0)
        asset_impact = asset.current_value * impact_percent * asset_impact_multiplier
        affected_assets.append({"symbol": asset.symbol, "impact": round(asset_impact, 2)})
...
async def get_risk_alerts(
    ...
    alerts = [
        {"id": "alert-1", "type": "concentration", "severity": "warning", "title": "High Sector Concentration", "description": "Technology sector allocation exceeds 35% of portfolio", "recommendation": "Consider rebalancing to reduce tech exposure", "created_at": "2026-02-02T10:30:00Z"},
        {"id": "alert-2", "type": "volatility", "severity": "info", "title": "Increased Market Volatility", "description": "VIX index has risen 15% in the past week", "recommendation": "Monitor positions closely; consider protective puts", "created_at": "2026-02-01T14:20:00Z"},
        {"id": "alert-3", "type": "correlation", "severity": "low", "title": "High Asset Correlation", "description": "Several holdings show correlation > 0.8", "recommendation": "Diversify into uncorrelated asset classes", "created_at": "2026-01-30T09:15:00Z"},
    ]
